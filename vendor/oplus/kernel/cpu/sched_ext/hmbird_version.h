...
enum hmbird_version {
	HMBIRD_GKI_VERSION,
	HMBIRD_OGKI_VERSION,
	HMBIRD_UNKONW_VERSION,
};

static enum hmbird_version hmbird_version_type = HMBIRD_UNKONW_VERSION;
...
static inline enum hmbird_version get_hmbird_version_type(void)
{
	...
	if (HMBIRD_UNKONW_VERSION != hmbird_version_type)
	...
	if (np) {
		...
		if (NULL != hmbird_version_str) {
			if (strncmp(hmbird_version_str, "HMBIRD_OGKI", strlen("HMBIRD_OGKI")) == 0) {
				...
				printk("hmbird version use HMBIRD_OGKI_VERSION,set by dtsi");
			} else if (strncmp(hmbird_version_str, "HMBIRD_GKI", strlen("HMBIRD_GKI")) == 0) {
				hmbird_version_type = HMBIRD_GKI_VERSION;
				printk("hmbird version use HMBIRD_GKI_VERSION,set by dtsi");
			} else {
				hmbird_version_type = HMBIRD_GKI_VERSION;
				printk("hmbird version use default HMBIRD_GKI_VERSION,set by dtsi");
			}
			return hmbird_version_type;
		}
	}
	...
	hmbird_version_type = HMBIRD_GKI_VERSION;
	printk("hmbird version use default GKI_VERSION");
	return hmbird_version_type;
}
